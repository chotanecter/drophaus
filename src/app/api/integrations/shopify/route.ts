...
export async function POST(req: NextRequest) {
  ...
  switch (body.action) {
    ...
    case 'enable_tracking': {
      /**
       * Enable inventory tracking on ALL existing Shopify product variants.
       * Sets inventory_management: 'shopify' on every variant that doesn't already have it.
       * This is a one-time fix for products created before the inventory_management field
       * was added to syncProductToShopify().
       */
      if (!SHOPIFY_DOMAIN || !ADMIN_TOKEN) {
        return NextResponse.json({ error: 'No Shopify Admin credentials configured' }, { status: 500 })
      }
      ...
      const details: { variantId: string; product: string; status: string; error?: string }[] = []

      for (const product of products) {
        for (const variant of product.variants) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const invMgmt = (variant as any).inventory_management
          if (invMgmt === 'shopify') {
            alreadyTracked++
            continue
          }
          ...
          try {
            const res = await fetch(
              `https://${SHOPIFY_DOMAIN}/admin/api/${API_VERSION}/variants/${variant.id}.json`,
              {
                ...
                body: JSON.stringify({
                  variant: { id: Number(variant.id), inventory_management: 'shopify' }
                }),
              }
            )
            ...
            if (res.ok) {
              updatedCount++
              details.push({ variantId: String(variant.id), product: product.title, status: 'updated' })
            } else {
              const errText = await res.text()
              failedCount++
              details.push({ variantId: String(variant.id), product: product.title, status: 'failed', error: errText })
            }

            // Rate limit: Shopify REST Admin = 2 req/sec bucket
            await new Promise(r => setTimeout(r, 550))
          } catch (err) {
            failedCount++
            details.push({ variantId: String(variant.id), product: product.title, status: 'failed', error: String(err) })
          }
        }
      }
      ...
      return NextResponse.json({
        ...
        totalVariants: updatedCount + failedCount + alreadyTracked,
